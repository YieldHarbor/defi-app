module YieldHarbor::vault {
    ...
    use YieldHarbor::epoch_time_lock::{ Self, EpochTimeLock};
    use YieldHarbor::oracle::{Self, Feed};
    use YieldHarbor::staked_sui::{ Self, StakedSui }; // clones of staking_pool.move
    use YieldHarbor::marketplace::{Self, Marketplace };
    use YieldHarbor::amm::{Self, Pool };
    ...
    struct Reserve has key {
        ...
        yt: Supply<TOKEN<YT>>,
        feed : Feed,
        marketplace: Marketplace<TOKEN<HYT>>,
        amm: Pool<TOKEN<YT>>,
    ...
    public entry fun new_vault(
        ...
        // setup HYT
        let pt = balance::create_supply(TOKEN<HYT> {});
        // setup YT
        ...
        let reserve = Reserve {
            ...
            yt,
            balance: 0,
            marketplace: marketplace::new_marketplace<TOKEN<HYT>>(ctx),
    ...
    public entry fun list(
        reserve: &mut Reserve,
        item: &mut Coin<TOKEN<HYT>>,
        amount: u64,
        price: u64,
        ctx: &mut TxContext
    ) {
        marketplace::list<TOKEN<HYT>>(&mut reserve.marketplace, item, amount, price, ctx);
    }
    ...
    public entry fun delist(
        ...
        marketplace::delist<TOKEN<HYT>>(&mut reserve.marketplace, order_id, ctx);
    }
    ...
    public entry fun buy(
        ...
        marketplace::buy<TOKEN<HYT>>(&mut reserve.marketplace, order_id, base_amount, payment, ctx);
    }

    public entry fun order_price(reserve: &Reserve, order_id: u64): u64 {
        marketplace::order_price<TOKEN<HYT>>(&reserve.marketplace, order_id)
    }

    public entry fun order_amount(reserve: &Reserve, order_id: u64): u64 {
        marketplace::order_amount<TOKEN<HYT>>(&reserve.marketplace, order_id)
    }
