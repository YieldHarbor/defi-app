#[test_only]
module YieldHarbor::marketplace_tests {
 
    use sui::coin::{Self};  
    use sui::sui::SUI; 
    use YieldHarbor::vault::{Self, Reserve, ManagerCap, TOKEN, HYT  };
    use sui::test_scenario::{Self as test, Scenario, next_tx, ctx};

    const MINT_AMOUNT: u64 = 10000000000; // 10 HYT
    const SELL_AMOUNT: u64 = 1000000000; // 1 HYT
    const PRICE: u64 = 950000000; // 1 HYT = 0.95 SUI
    ...
    fun test_list_delist_(test: &mut Scenario) {
        ...
        next_tx(test, seller);
        {
            ...
            vault::new_vault(&mut managercap,  10, MINT_AMOUNT, &mut sui_token, ctx(test));
        ...
        next_tx(test, seller);
        {
            ...
            let mock_vault_token = coin::mint_for_testing<TOKEN<HYT>>(MINT_AMOUNT, ctx(test));
